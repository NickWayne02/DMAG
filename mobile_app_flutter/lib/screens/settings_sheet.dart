...
class SettingsSheet extends StatefulWidget {
  ...
  static Future<void> show(BuildContext context) {
    return showGeneralDialog(
      ...
      barrierColor: Colors.black.withOpacity(0.7), // Darker background to compensate for removed blur
...
class _SettingsSheetState extends State<SettingsSheet> {
  ...
  void initState() {
    ...
    if (tp.customAccent != null) {
      _hexController.text = '#${tp.customAccent!.value.toRadixString(16).padLeft(8, '0').substring(2).toUpperCase()}';
    } else {
      _hexController.text = '#${tp.activeAccent.primary.value.toRadixString(16).padLeft(8, '0').substring(2).toUpperCase()}';
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (themeProvider.customAccent != null && !FocusScope.of(context).hasFocus) {
      _hexController.text = '#${themeProvider.customAccent!.value.toRadixString(16).padLeft(8, '0').substring(2).toUpperCase()}';
    } else if (themeProvider.customAccent == null && !FocusScope.of(context).hasFocus) {
      _hexController.text = '#${themeProvider.activeAccent.primary.value.toRadixString(16).padLeft(8, '0').substring(2).toUpperCase()}';
    }
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Text(
              'Персонализация: язык, тема и размеры интерфейса.',
              style: GoogleFonts.inter(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Все тексты автоматически переводятся на выбранный язык.',
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text('Тема', style: GoogleFonts.inter(color: colors.foreground, fontSize: 16, fontWeight: FontWeight.bold)),
                    ],
                  ),
                  const SizedBox(height: 16),
                  Row(
                    children: [
                      Expanded(child: _buildModeOption(context, themeProvider, LucideIcons.sun, 'Светлая', ThemeModeType.light)),
                      const SizedBox(width: 8),
                      Expanded(child: _buildModeOption(context, themeProvider, LucideIcons.moon, 'Тёмная', ThemeModeType.dark)),
                      const SizedBox(width: 8),
                      Expanded(child: _buildModeOption(context, themeProvider, LucideIcons.zap, 'AMOLED', ThemeModeType.neon)),
                      const SizedBox(width: 8),
                      Expanded(child: _buildModeOption(context, themeProvider, LucideIcons.sliders_horizontal, 'Своя', ThemeModeType.custom)),
                    ],
                  ),
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text('Цветовая тема', style: GoogleFonts.inter(color: colors.foreground, fontSize: 16, fontWeight: FontWeight.bold)),
                    ],
                  ),
                  const SizedBox(height: 16),
                  GridView.count(
                  ...
                  BounceButton(
                    ...
                    child: Container(
                      ...
                      alignment: Alignment.center,
                      child: Text('Готово', style: GoogleFonts.inter(color: colors.primaryForeground, fontWeight: FontWeight.w600)),
                    ),
                  ),
                  const SizedBox(height: 12),
                  BounceButton(
                    onTap: () {
                      themeProvider.setMode(ThemeModeType.neon);
                    ...
                    child: Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          const SizedBox(width: 8),
                          Text('Сбросить', style: GoogleFonts.inter(color: colors.foreground, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  ),
  ...
  Widget _buildModeOption(BuildContext context, ThemeProvider provider, IconData icon, String label, ThemeModeType mode) {
    ...
    return BounceButton(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            Icon(icon, color: active ? primary : colors.foreground.withOpacity(0.5), size: 20),
            const SizedBox(height: 8),
            Text(
              label,
              style: GoogleFonts.inter(
                color: active ? primary : colors.foreground.withOpacity(0.5),
                fontSize: 10,
      ...
      children: [
        Row(
          ...
          children: [
            ...
            Text(middle, style: GoogleFonts.inter(color: colors.foreground.withOpacity(0.5), fontSize: 14)),
          ],
        ),
        const SizedBox(height: 16),
        SliderTheme(
        ...
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(left, style: GoogleFonts.inter(color: colors.foreground.withOpacity(0.5), fontSize: 10)),
            Text(right, style: GoogleFonts.inter(color: colors.foreground.withOpacity(0.5), fontSize: 10)),
          ],
        )
      ],
    );
